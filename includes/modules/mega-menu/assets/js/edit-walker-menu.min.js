...
(function ($, wind) {
  ...
  window.navMenuSettings = {
        ...
        $.each(mutations, function (index, mutation) {
          if (mutation.type == 'childList') {
            if (mutation.addedNodes.length) {
              var item = mutation.addedNodes[0];
    ...
    initNavSettings: function initNavSettings() {
      var _this = this;
      ...
      var elSelectTypeIcon = $('.portuna-addon-icon-type');
      var elIconColor = $('.portuna-addon-color');
      var elCheckbox = $('.portuna-addon-mega-menu-checkbox');
      ...
      tail.select('.portuna-addon-select-icon', {
        ...
        cbComplete: function cbComplete(item) {
          var labels = $(item).find('.select-label');
          ...
          for (var i = 0; i <= getList; i++) {
            var getCurrent = $(".dropdown-option:eq(".concat(i, ")"));

            if (labels.text() === getCurrent.find('.text-name').text()) {
              var getKey = getCurrent.data('key'),
                  getName = getCurrent.find('.text-name').text();
              labels.html("<span class=\"label-inner\"><i class=\"".concat(getKey, "\" style=\"margin-right: 3px;\"></i>").concat(getName, "</span>"));
            }
          }
        },
        cbLoopItem: function cbLoopItem(item, group, search) {
          var li = document.createElement("LI");
          li.className = "dropdown-option" + (item.selected ? " selected" : "") + (item.disabled ? " disabled" : "");

          if (search && search.length > 0 && _this.con.searchMarked) {
          ...
          li.innerHTML = "<div class='item-inner'>" + "    <div class='item-text'>" + "        <i class='" + icon + "'></i>" + "        <span class='text-name'>" + name + "</span>" + "    </i>" + "</div>";
          return li;
        }
      });
      $('.portuna-addon-mega-menu').each(function () {
        var currentIcon = $(this).find('.portuna-addon-select-icon');
        var currentClosest = $(this).closest('li.portuna-addon-mega-menu');
        tail.select(currentIcon[0]).on('change', function (item) {
          var labels = currentClosest.find('.field-is-icon-lib .select-label');
          var getKey = item.key,
              getName = item.value;
          labels.html("<span class=\"label-inner\"><i class=\"".concat(getKey, "\" style=\"margin-right: 3px;\"></i>").concat(getName, "</span>"));
        });
    ...
    uploadCustomImage: function uploadCustomImage() {
      var uploadCustomIcon = function uploadCustomIcon(item_id, thumb_id) {
        wp.media.post('portuna-addon-menu-set-thumbnail', {
          ...
          $("#field-is-icon-cus-".concat(item_id)).html(html);
        });
      };

      $('#menu-to-edit').on('click', '.menu-item .portuna-addon-set-custom-icon', function (e) {
        e.preventDefault();
        e.stopPropagation();
        ...
        e.preventDefault();
        e.stopPropagation();
        var item_id = $(this).parent().find('.portuna-addon-icon-src').val();
        uploadCustomIcon(item_id, -1);
      });
    },
    editMegaMenu: function editMegaMenu() {
      $('#menu-to-edit').on('click', '.menu-item .portuna-addon-mega-menu-edit-btn', function () {
        $('body, html').css('overflow', 'hidden');
        $('.portuna-mega-menu-wrapper').show();
        var adminUrl = customIcon.rest_url,
            key_id = $('#menu').val(),
        ...
        $('#portuna-mega-menu-iframe').on('load', function () {
          $('.portuna-mega-menu-popup-close').show();
        });
      }); 

      $('.portuna-mega-menu-popup-close').on('click', function () {
        $(this).hide();
