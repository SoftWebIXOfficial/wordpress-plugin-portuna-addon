...
(function (root, factory) {
  if (typeof define === "function" && define.amd) {
    ...
    if (typeof MooTools !== "undefined") {
      Element.implement({
        tailselect: function tailselect(o) {
  ...
  function _trigger(el, event, opt) {
  ...
  var select = function select(el, config) {
  ...
  select.strings = {
    ...
    ru: {
      ...
      placeholderMulti: function placeholderMulti(args) {
    ...
    modify: function modify(locale, id, string) {
      if (!(locale in this)) {
        return false;
      }
    ...
    register: function register(locale, object) {
  ...
  select.prototype = {
    _e: function _e(string, replace, def) {
    ...
    init: function init() {
      ...
      for (var key in con.items) {
        ...
        this.options.add(con.items[key].key || key, con.items[key].value, con.items[key].group, con.items[key].selected, con.items[key].disabled, con.items[key].description);
      }
    ...
    callback: function callback(item, state, _force) {
    ...
    trigger: function trigger(event) {
      ...
      if (event == "change" && arguments[2] && arguments[2].indexOf("select") >= 0) {
        _trigger(this.e, "input", obj);

        _trigger(this.e, "change", obj);
      }

      _trigger(this.select, "tail::" + event, obj);
    ...
    query: function query(search, conf) {
    ...
    cbGroup: function cbGroup(group, search) {
    ...
    cbItem: function cbItem(item, optgroup, search) {
    ...
    updateLabel: function updateLabel(label) {
      ...
      label = '<span class="label-inner">' + label + '</span>', limit = c.multiShowLimit && c.multiLimit < Infinity;
    ...
    updateContainer: function updateContainer(item) {
    ...
    updatePin: function updatePin(item) {
    ...
    updateCSV: function updateCSV(item) {
    ...
    config: function config(key, value, rebuild) {
    ...
    disable: function disable(update) {
    ...
    on: function on(event, callback, args) {
  ...
  options.prototype = {
    ...
    add: function add(key, value, group, selected, disabled, description, rebuild) {
      if (key instanceof Object) {
        for (var k in key) {
    ...
    move: function move(item, group, new_group, rebuild) {
    ...
    remove: function remove(item, group, rebuild) {
      if (!(item = this.get(item, group))) {
        return false;
      }
    ...
    is: function is(state, key, group) {
    ...
    handle: function handle(state, key, group, _force) {
    ...
    enable: function enable(key, group) {
      return this.handle("enable", key, group, false);
    },
    disable: function disable(key, group) {
      return this.handle("disable", key, group, false);
    },
    select: function select(key, group) {
      return this.handle("select", key, group, false);
    },
    unselect: function unselect(key, group, _force) {
      return this.handle("unselect", key, group, _force);
    },
    toggle: function toggle(item, group) {
      if (!(item = this.get(item, group))) {
        return false;
      }
    ...
    walk: function walk(state, items, args) {
    ...
    applyLinguisticRules: function applyLinguisticRules(search, casesensitive) {
    ...
    find: function find(search, config) {
      var self = this,
          matches,
          has = {}; 
      ...
      if (typeof config === "function") {
        matches = config.bind(this, search);
          ...
          var regex = new RegExp(search, !has["case"] ? "mi" : "m"),
              sfunc = function sfunc(opt) {
            return regex.test(opt.text || opt.value);
          }; 


          if (has.any) {
            matches = function matches(opt) {
    ...
    finder: function finder(search, config) {
    ...
    walker: function walker(orderi, orderg) {
